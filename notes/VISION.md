...
- **No classes, no `match` (yet).** Monty's Python subset is evolving. We don't work around it — we wait. Context managers are available in Monty v0.0.18, but host-sensitive uses such as `with open(...)` remain sandbox-controlled by fimod.
- **Deny-by-default capabilities, limits-by-default resources.** Pre-0.5.0, every `OsCall` is denied (`None`) — keeps the security story simple at the cost of ergonomic paper cuts (`datetime.now() → None` silently). From 0.5.0, the sandbox splits the two concerns: capabilities (clock, env, FS) stay deny-by-default and require explicit opt-in via `sandbox.toml`; resource limits (CPU, memory) get reasonable hard-coded defaults (`10m` / `2GB`) that protect the host even without a policy file. See `notes/monty-default-sandboxing.md`.
