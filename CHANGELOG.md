...
- ⚡ **Leaner data paths** — dotpath edits, record helpers, and compatible outputs avoid unnecessary cloning and JSON conversion.
...
- **dotpath:** mutate owned snapshots without cloning ancestor subtrees while preserving independent results and object order.
- **iter helpers:** retain JSON-native records through sort, count, minimum, and maximum operations instead of normalizing every field through `serde_json::Value`.
- **output:** select direct serializers after final format and destination resolution, extending the allocation-saving path to compatible mold overrides and inferred formats.
...
- **deps deferred:** defer the `serde-saphyr` 0.0.28 patch bump to the next dependency PR. The final freshness gate found it only after the release PR had merged, so 0.9.0 ships the already-validated 0.0.27 YAML stack instead of reopening the release train for a patch-level dependency refresh.
...
- **deps:** defer the `reqwest` 0.13.4 patch bump to the next dependency PR. The final freshness gate found it only after the release PR had merged, so 0.8.0 ships the already-validated 0.13.3 stack instead of reopening the release train for a patch-level dependency refresh.
